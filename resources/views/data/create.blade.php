    ...
    <div class="page-content">
        <div class="container-fluid">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <a href="{{ route('data.new.create') }}" class="btn btn-primary">
                    <i class="fas fa-plus me-1"></i> Input Komponen Baru
                </a>
            </div>
            <div class="row">
                <div class="col-12">
                        ...
                        <div class="card-body">
                            <form action="{{ route('data.store') }}" method="POST" id="dataForm" autocomplete="off">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('line') is-invalid @enderror" id="line"
                                        name="line">
                                        <option value="" disabled
                                            {{ old('line', session('line')) ? '' : 'selected' }}>Select Line</option>
                                        @foreach ($lines as $line)
                                            <option value="{{ $line }}"
                                                {{ old('line', session('line')) == $line ? 'selected' : '' }}>
                                                {{ $line }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('no_station') is-invalid @enderror" id="no_station"
                                        name="no_station">
                                        <option value="" disabled
                                            {{ old('no_station', session('no_station')) ? '' : 'selected' }}>Select No
                                            Station</option>
                                        @foreach ($noStations as $noStation)
                                            <option value="{{ $noStation }}"
                                                {{ old('no_station', session('no_station')) == $noStation ? 'selected' : '' }}>
                                                {{ $noStation }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select @error('nama_station') is-invalid @enderror"
                                        id="nama_station" name="nama_station">
                                        <option value="" disabled
                                            {{ old('nama_station', session('nama_station')) ? '' : 'selected' }}>Select
                                            Nama Station</option>
                                        @foreach ($namaStations as $namaStation)
                                            <option value="{{ $namaStation }}"
                                                {{ old('nama_station', session('nama_station')) == $namaStation ? 'selected' : '' }}>
                                                {{ $namaStation }}
                                            </option>
                                        @endforeach
                                    </select>
                    ...
                    fetch(`/data/no-stations/${encodeURIComponent(line)}`)
                        ...
                        .then(data => {
                            if (data.no_stations.length === 0) {
                                noStationSelect.innerHTML =
                                    '<option value="" disabled>No No Station Available</option>';
                            } else {
